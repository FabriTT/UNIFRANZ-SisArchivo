    ...
    @section('content')
        <div class="card">
            <div class="card-header">
                <div class="card-title text-center"><span class="h2">ADMINISTRACION DE DOCUMENTOS COMPLEMENTARIOS</span>
                </div>
            </div>
            <div class="card-body d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <form method="POST" action="{{ route('documentoBuscar') }}" class="form-inline" name="form-buscar"
                        id="form-buscar">
                        @csrf
                ...
                <div class="table-responsive">
                    <table class="table table-striped text-center">
                        <thead style="background: #222222; color: white">
                            ...
                            <th scope="col">Documentos complementarios registrados</th>
                        ...
                        @forelse ($docentes as $docente)
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{ $docente->cantidad_de_documentos }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-icon btn-round btn-warning"
                                            data-toggle="modal" data-target="#Documentos"
                                            onclick="Mostrar('{{ $docente->Nombres_doc . ' ' . $docente->Paterno_doc . ' ' . $docente->Materno_doc }}','{{ $docente->Carnet_doc }}','{{ route('historialDocumentos') }}','{{ asset('storage/') }}')">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title mx-auto"><strong>Registro de documentos complementario</strong>
                        </h4>
                    </div>


                    <div class="modal-body">
                        <form method="POST" name="form-documento" id="form-documento">
                            @csrf
                            @can('saveDocumento')
                                <div class="form-row">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        ...
                                        <input type="text" class="form-control" name="descripcion" id="descripcion" maxlength="80">
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        <label for="nombre">*Fecha de emision:</label>
                                        <input type="date" class="form-control" name="fecha" id="fecha" max="{{ now()->format('Y-m-d') }}">
                                    </div>
                                    <div class="form-group col-md-6">
                            ...
                            <div class="table-responsive scroll" style="max-height: 400px;">
                                <table class="table table-striped text-center" id="TablaDocumentos"
                                    name="TablaDocumentos">
                                    <thead style="background: #222222; color: white">
                                        <tr>
                            ...
                            <div class="d-flex align-items-center">
                                @can('saveDocumento')
                                    <button type="button" class="btn btn-success"
                                        onclick="Insertar('{{ route('saveDocumento') }}','{{ route('documento_complementario') }}','{{ route('contrato') }}')">Guardar</button>
                                @endcan
        ...
        <script>
            ...
            var historial = '{{ route('historialDocumentos') }}';
            var rutaBorrar = '{{ route('borrarDocumento') }}';
            var rutaCargar = '{{ route('documento_complementario') }}';
            var permisoBorrarDocumento = ' {{ auth()->user()->hasPermissionTo('borrarDocumento') }}';
        </script>


        <script src="{{ asset('propio/Complementarios.js') }}"></script>
