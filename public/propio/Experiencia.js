    ...
    if (showModal === "1") {
        ...
        if (jQuery("#Experiencia").length > 0) {
            ...
            $.ajax({
                ...
                jQuery("#Modaños").val(docente[0].Años_experiencia_doc);
            });
        } else {
...
function seleccionar(datos) {
    ...
    document.getElementById("Modaños").value = datosObj.Años_experiencia_doc;
}
...
function Modificar(ruta, ruta2, ruta3) {
    ...
    var fotocurriculum = document.getElementById("Modfotocurriculum");
    var años = document.getElementById("Modaños");
    var fotoexperiencia = document.getElementById("Modfotoexperiencia");

    var validado = true;

    //Validar Vacios
    validado = validado && validarVacio(fotocurriculum, contenedor, mensaje);
    validado = validado && validarVacio(años, contenedor, mensaje);
    validado = validado && validarVacio(fotoexperiencia, contenedor, mensaje);

    //validar numeros
    validado = validado && validarNumeros(años, contenedor, mensaje);

    //validar archivos
    validado = validado && validarArchivos(fotocurriculum, contenedor, mensaje);
    validado =
        validado && validarArchivos(fotoexperiencia, contenedor, mensaje);
...
function HistoCurriculum(datos, ruta, direccion, rutaBorrar) {
    var datosObj = JSON.parse(datos);
    var carpeta =
    ...
    $.ajax({
        ...
        var tabla = document.getElementById("TablaCurriculum");
...
function HistoExperiencia(datos, ruta, direccion, rutaBorrar) {
    var datosObj = JSON.parse(datos);
    var carpeta =
        datosObj.Nombres_doc.toUpperCase() +
        " " +
    ...
    $.ajax({
        ...
        var tabla = document.getElementById("TablaExperiencia");
        ...
        for (var i = 0; i < archivos.length; i++) {
            ...
            (function (indice) {
                boton.onclick = function () {
                    borrarArchivo(
                        carpeta +
                            "/RESPALDOS DE EXPERIENCIA LABORAL/" +
                            archivos[indice].nombre +
                            "." +
                            archivos[indice].extension,
                        rutaBorrar
